...
function [rms_pos, rms_vel] = run_trajectory_readonly(h1, h2, h3, h4, h5, h6, h7, h8, h9, h10,trajectory_generator)
...
while (1)
    ...
    % added by jixin to computer RMS
    num_iterations = num_iterations + 1;
    rms_pos = rms_pos + sum((true_s(1:3) - des_s(1:3)).^2);
    rms_vel = rms_vel + sum((true_s(4:6) - des_s(4:6)).^2);
    
    if time >= time_tol
        rms_pos = sqrt( rms_pos/num_iterations );
        rms_vel = sqrt( rms_vel/num_iterations );
        break;
    end;    
