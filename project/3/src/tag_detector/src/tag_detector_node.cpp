...
void process(const vector<int> &pts_id, const vector<cv::Point3f> &pts_3, const vector<cv::Point2f> &pts_2, const ros::Time& frame_time)
{
    ...
    // recurrent rms:  final rms:   0.232831    0.193553    0.585309
    /*
...
void img_callback( const sensor_msgs::ImageConstPtr& img_msg )
{
    ...
    std::cout << "  get marker num: " << TheBoardDetected.size( ) << std::endl;

    vector< cv::Point3f > pts_3;
    vector< cv::Point2f > pts_2;
    vector< int > pts_id;
    for ( unsigned int i = 0; i < TheBoardDetected.size( ); i++ )
    {

        int idx = TheBoardDetected.conf.getIndexOfMarkerId( TheBoardDetected[i].id );
        ...
        char str[100];
        sprintf( str, "%d", idx );
        cv::putText( bridge_ptr->image, str, TheBoardDetected[i].getCenter( ), CV_FONT_HERSHEY_COMPLEX, 0.4, cv::Scalar( -1 ) );
        for ( unsigned int j = 0; j < 4; j++ )
        {
            sprintf( str, "%d", j );
            cv::putText( bridge_ptr->image, str, TheBoardDetected[i][j], CV_FONT_HERSHEY_COMPLEX, 0.4, cv::Scalar( -1 ) );
        }

        aruco::MarkerInfo Minfo = TheBoardDetected.conf.getMarkerInfo( TheBoardDetected[i].id );
        for ( unsigned int j = 0; j < 4; j++ )
        {
            pts_id.push_back( TheBoardDetected[i].id * 4 + j );
            pts_2.push_back( TheBoardDetected[i][j] );
            pts_3.push_back( Minfo.at( j ) * MarkerSize / 1000 );
        }
    }
